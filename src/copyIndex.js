/*
- Just a copy of the original 'index.js' file.
- We'll be using this as a reference to look back on in case we 
	have problems, and ultimately it'll help speed up development somewhat
...
// import {createOverlay, createModal, createPageHeader, createPageSidebar, createMainContentSection, createPageFooter} from "./initialPage.js";
...
const DOMElementsModule = (() => {
    ...
    const modalMainContentSection = modalEl.querySelector(
        ".modal-main-content"
    );
    ...
    const projectSidebar = createPageSidebar();
    ...
    const projectFooter = createPageFooter();
    ...
    todoForm.addEventListener("submit", (e) => {
        ...
        const inputDueDate = document.getElementById(
            "todo-due-date-field"
        ).value;
        const inputPriority = document.getElementById(
...
function displayTodoForm(e) {
    ...
    const inputDescriptionEl = document.getElementById(
        "todo-description-field"
    );
    ...
    formInfoModule.activeTodoIndex =
        e.currentTarget.parentElement.parentElement.dataset.todoIndex;
    ...
    if (formInfoModule.isEdit) {
        const currentTodo =
            currentProject.projectTodos[formInfoModule.activeTodoIndex];
...
function displayTodoDetails(e) {
    ...
    if (typeof formInfoModule.activeTabID === "string") {
        ...
        currentTodo =
            projectsList[formInfoModule.activeTabID].projectTodos[todoIndex];
    }
    ...
    todoDueDateEl.textContent = `Due Date: ${currentTodo.dueDate
        .toISOString()
        .slice(0, 10)}`;
}
...
function updateSidebarTabs() {
    ...
    projectsTabSection.innerHTML = projectsList
        .map((project, index) => {
    ...
    for (let i = 0; i < projectsList.length; i++) {
        projectsList[i].projectTodos = sortTodosByDate(
            projectsList[i].projectTodos
        );
    }
    ...
    for (let i = 0; i < projectsList.length; i++) {
        ...
        for (let j = 0; j < projectTodos.length; j++) {
            ...
            if (isSameDate(currentDate, projectTodos[j].dueDate)) {
                ...
                isSevenDaysInFuture(currentDate, projectTodos[j].dueDate)
            ) {
    ...
    const sidebarTabs = DOMElementsModule.projectSidebar.querySelectorAll(
        "li.sidebar-tab-item"
    );
...
function renderMainContent() {
    const activeTab = document.querySelector(
        "li.sidebar-tab-item[data-active='true']"
    );
    ...
    if (tabType == "mainTab") {
        ...
        todoListContainer.innerHTML = todos
            .map((todo, index) => {
                return `<li class="todo-item" data-todo-complete="${
                    todo.isComplete.getCompletion() ? "true" : "false"
        ...
        todoListContainer.innerHTML = todos
            .map((todo, index) => {
                return `<li class="todo-item" data-todo-complete="${
					...
					<span class="todo-due-date-el">Due: ${formatDateToUS(todo.dueDate)}</span>
					<div class="todo-btns">
						<button class="todo-btn toggle-todo-btn green-btn">${
...
function toggleTodoCompletion(e) {
    ...
    const currentTodo =
        projectsList[formInfoModule.activeTabID].projectTodos[
            todoElement.dataset.todoIndex
        ];
    ...
    if (currentTodo.isComplete.getCompletion()) {
        ...
        projectsList[formInfoModule.activeTabID].projectTodos[
            todoElement.dataset.todoIndex
        ].isComplete.setCompletion(false);
    } else {
        projectsList[formInfoModule.activeTabID].projectTodos[
            todoElement.dataset.todoIndex
        ].isComplete.setCompletion(true);
    }
...
function deleteTodo(e) {
    ...
    projectsList[formInfoModule.activeTabID].projectTodos.splice(
        todoElement.dataset.todoIndex,
        1
    );
    localStorage.setItem("projects", JSON.stringify(projectsList));
